...
The TidyText R package is being utilized for most of this analysis:
...
Here I'm making sure that out of school pertains to youth and not after school programs
...
quadgrams.region <- pdf.text %>%
  unnest_tokens(quadgram, Text, token = "ngrams", n = 4) %>%
  filter(!is.na(quadgram)) %>%
  count(quadgram, Region, sort = TRUE)
...
- Opportunity Youth also includes OY. 

- Disconnected Youth also includes DY.

-- There are no mentions of "youth that are disconnected", "disconnected teenagers", "disconnected young people", or "youth disconnection"

- Out of School includes out of school youth, out of school youths, OSY, OS, dropout, drop out, dropouts, and drop out of school.

-- "nor in school" had zero mentions


Alamo used the phrase "bachelor's degree none" at least 88 times, which may be related to opportunity youth.


```{r OY Phrases, echo=FALSE}

# Phrases to add to search:
# -	juvenile justice (criminal, incarceration)
# -	young adult
# -	And childcare reskill/upskill/apprenticeship 
# -	Nonprofit partnerships
# -	Rural related keywords
# -	Adult literacy
# -	Mental health
# -	Maybe: emerging adults, teenagers, young adults
# -	Maybe: neither employed nor in school
...
oy.phrases <- quadgrams.region %>% 
                                # The problem here is that "opportunity youth oy" will be double counted  
  mutate(OY = ifelse(endsWith(quadgram, "opportunity youth") |
                       endsWith(quadgram, " oy"), n, 0),
         OSY = ifelse(endsWith(quadgram, "out of school youth") |
                        endsWith(quadgram, " os") |
                        endsWith(quadgram, " osy") |
                        endsWith(quadgram, "out of school youths") |
                        endsWith(quadgram, "drop out") |
                        endsWith(quadgram, "dropout") |
                        endsWith(quadgram, "dropouts") |
                        endsWith(quadgram, "drop out of school"), n, 0),
         DY = ifelse(endsWith(quadgram, "disconnected youth") |
                       endsWith(quadgram, " dy"), n, 0)) %>% 
  group_by(Region) %>% 
  summarise("Opportunity Youth" = sum(OY), 
            "Disconnected Youth" = sum(DY),
            "Out of School" = sum(OSY)) %>% 
  mutate("Total Mentions" = `Opportunity Youth` + `Disconnected Youth` + `Out of School`) %>% 
  arrange(desc(`Total Mentions`)) %>% 
  mutate(Rank = row_number())
# AMY START HERE: Next divide by  n/4? (because we're looking at quadgrams?)

kable(oy.phrases, caption = "Any Related Opportunity Youth Mentions")
...
bigram.homeless <- bigrams %>% 
  mutate(homeless = ifelse(startsWith(bigram, "homeless"), 1, 0)) %>%
  group_by(bigram) %>%   
  summarise(homeless = sum(homeless)) %>% 
  filter(homeless > 2) %>% 
  arrange(-homeless)

kable(bigram.homeless,
      caption = "Words following homeless (with more than two mentions)")
```

Only three regions directly mention homeless youth. One example is the Panhandle:

"In addition to continuing to serve low-income youth, <homeless youth>, youth who have disabilities, foster youth, and youth who have barriers to employment, refocusing efforts to outreach and *engage out of school youth in successful work-based training opportunities is the primary goal of the PWDB*, related to youth services."

The homeless counts below are for any mention of homeless, not just youth related. Even so, only 21 regions have mentions.

- Homeless includes homeless and homelessness.
...
Homeless.new <- quadgrams.region %>% 
  mutate(Homeless = ifelse(endsWith(quadgram, "homeless") |
                             endsWith(quadgram, "homelessness"), n, 0),
         Unhoused = ifelse(endsWith(quadgram, "unhoused"), n, 0)) %>% 
  group_by(Region) %>% 
  summarise(Homeless = sum(Homeless)) %>% 
  arrange(desc(`Homeless`)) %>% 
  mutate(Rank = row_number())
         
 kable(Homeless.new, caption = "Homeless Mentions")
