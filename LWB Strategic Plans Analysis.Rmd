...
# Below I unnested earlier. This doesn't capture line numbers.
...
word.count.region.25.long <- word.count.region.25 %>% 
  pivot_longer(cols = Cameron:`Permian-Basin`, names_to = "Region", values_to = "count")
...
Opportunity was also mentioned by all regions, but I need to figure out how to search for "opportunity youth" together instead of as separate words.
...
sentiment.region <- words %>%
  inner_join(get_sentiments("bing")) %>%
  count(Region, index = linenumber %/% 10, sentiment) %>%
...
ggplot(sentiment.region, aes(index, sentiment, fill = Region)) +
  ...
  labs(title = "Sentiment of Strategic Plans by Region",
       subtitle = "by every 10 lines of narrative")
